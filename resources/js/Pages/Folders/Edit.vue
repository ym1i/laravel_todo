...
const title = ref(props.folderTitle)

const updateFolder = () => {
    router.post(`/folders/${props.folderId}/edit`, {
        title: title.value,
    })
}
...
<template>
    ...
    <div class="grid grid-cols-5">
        ...
        <div class="mt-5 col-start-3 col-span-2 row-start-1 row-span-1 w-full max-w-sm p-4 bg-white border border-gray-200 rounded-lg shadow sm:p-6 md:p-8">
            <form @submit.prevent="updateFolder" class="space-y-6">
                <h5 class="text-xl font-medium text-gray-900 ">Edit Folder</h5>
                <div class="flex">
                    ...
                    <input v-model="title" type="text" name="title" placeholder="Edit Folder" class="rounded-none rounded-e-lg bg-gray-50 border border-gray-300 text-gray-900 focus:ring-blue-500 focus:border-blue-500 block flex-1 min-w-0 w-full text-sm p-2.5">
                </div>
